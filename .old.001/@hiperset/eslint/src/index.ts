...
import { HipersetValidator } from "@hiperset/rules"


const createRule = ESLintUtils.RuleCreator(name => ``)

export const rule = createRule({
  name: 'no-nono-hiperset',
  meta: {
    type: 'problem',
    docs: { description: 'Proíbe o valor nono no label hiperset' },
    messages: { nonoDetected: 'O valor "{{val}}" é proibido em hiperset!' },
    schema: [], // sem opções
  },
  defaultOptions: [],
  create(context) {
    // Acessa as ferramentas que conectam ESLint ao TypeScript
    const parserServices = ESLintUtils.getParserServices(context);
    const checker = parserServices.program.getTypeChecker();

    return {
      // O ESLint percorre a árvore ESTree, mas nós queremos o nó do TS
      LabeledStatement(node) {
        // Converte o nó do ESLint para o nó real do TypeScript
        const tsNode = parserServices.esTreeNodeToTSNodeMap.get(node);

        if (HipersetValidator.isHipersetLabel(tsNode, ts)) {
          const invalidValue = HipersetValidator.getInvalidValue(tsNode, ts);
          if (invalidValue) {
            context.report({
              node,
              messageId: 'nonoDetected',
              data: { val: invalidValue },
            });
          }
        }
      },
...
// function checkIsLabeledStatement(ts: TS, node: ts.Node | undefined, labelName: string) {
